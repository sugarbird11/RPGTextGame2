...
public class Magician extends Hero {
	...
	public int getDamage() {
		if (this.weapon.equals("지팡이")) {
			return this.power * 3;
		} else if (this.weapon.equals("빛나는 지팡이")) {
			return this.power * 4;
		}
		return 0;
	}
	...
	public double select_attack(Monster monster) {
		String[] attacks = { "파이어 볼트", "일렉트릭 볼트", "아이스 볼트" };
		for (int i = 1; i <= attacks.length; i++) {
			if (i >= 3 && !this.weapon.equals("빛나는 지팡이"))
				continue;
		...
		if (numattack < 1 || numattack > (this.weapon.equals("빛나는 지팡이") ? attacks.length : 2)) {
